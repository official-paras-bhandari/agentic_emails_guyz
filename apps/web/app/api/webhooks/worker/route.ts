...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!signature || !timestamp) return NextResponse.json({ error: 'Missing signature or timestamp' }, { status: 401 });
    ...
    switch (type) {
      ...
      case 'lead_found':
        await handleLeadFound(job_id, data);
        break;

      case 'draft_created':
        await handleDraftCreated(job_id, data);
        break;
      ...
      case 'job_completed':
        ...
        // Ensure all steps are marked as completed when the job finishes
        await prisma.jobStep.updateMany({
      ...
      case 'job_failed':
        ...
        // Mark any currently running step as failed on job failure
        await prisma.jobStep.updateMany({
...
async function handleDraftCreated(jobId: string, data: any) {
  const job = await prisma.job.findUnique({ where: { id: jobId } });
  if (!job || data.workspace_id !== job.workspaceId) throw new Error('Draft event workspace mismatch');
  try {
    const followupTask = data.followup_task_id
      ? await prisma.followUpTask.findFirst({
          where: { id: data.followup_task_id, workspaceId: job.workspaceId },
    ...
    // If this was a follow-up draft, mark the FollowUpTask as completed
    ...
    // If this was a follow-up, revert task to scheduled so it retries
    if (data.followup_task_id) {
      await prisma.followUpTask.updateMany({
        where: { id: data.followup_task_id, workspaceId: job.workspaceId, status: 'processing' },
        data: { status: 'scheduled', scheduledFor: new Date(Date.now() + 30 * 60_000) }, // retry in 30 min
      });
    }
...
async function handleLeadFound(jobId: string, data: any) {
    const job = await prisma.job.findUnique({ where: { id: jobId } });
    if (!job) return;
    ...
    const canDiscover = await contactPolicyService.canDiscoverLead(job.workspaceId, {
        email,
        website: website_url
    });
    ...
    // 2. Dedupe check
    const existing = await dedupeService.findExistingLead(job.workspaceId, {
        email,
        website: website_url,
    ...
    if (existing) {
        ...
        // Attach source to existing lead if it's a new source URL
        if (source_url) {
